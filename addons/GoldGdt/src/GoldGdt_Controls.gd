...
## When false, this node skips all input processing.
var is_local_player: bool = true

func _ready() -> void:
	# Always set up input in _ready — the PlayerSpawner will call
	# configure_for_remote() on non-local pawns immediately after.
...
func _input(event) -> void:
	if not is_local_player:
		return
	# Skip all player input while paused (PauseMenu autoload handles Escape).
	if get_tree().paused:
		return
