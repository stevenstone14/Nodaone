...
func _physics_process(delta) -> void:
	# In multiplayer, only the local player should run physics.
	# Remote players' positions are synced via MultiplayerSynchronizer.
...
## Apply a rocket boost impulse to this body's velocity.
## Called when a rocket explodes nearby (Quake-style rocket jump).
func apply_rocket_boost(impulse: Vector3) -> void:
	velocity += impulse
	print("Rocket boost applied! Impulse: ", impulse, " New velocity: ", velocity)
